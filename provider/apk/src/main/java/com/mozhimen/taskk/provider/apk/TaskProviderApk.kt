...
open class TaskProviderApk(
    ...
    override fun getTaskVerify(): ATaskVerify {
        return TaskVerifyApk(_taskManagerProvider,_iTaskLifecycle)
    }

    override fun getTaskUnzip(): ATaskUnzip {
        return TaskUnzipApk(_taskManagerProvider,_iTaskLifecycle).apply {
    ...
    override fun getTaskInstall(): ATaskInstall {
        return TaskInstallApk(_taskManagerProvider,_iTaskLifecycle).apply {
            this.setInstallKReceiverProxy(InstallKManager.getInstallKReceiverProxy())
        }
    }

    override fun getTaskOpen(): ATaskOpen {
        return TaskOpenApk(_taskManagerProvider,_iTaskLifecycle)
    }
    ...
    override fun getTaskUninstall(): ATaskUninstall {
        return TaskUninstallApk(_taskManagerProvider,_iTaskLifecycle)
    }
        ...
        InstallKManager.apply {
            ...
            registerPackagesChangeListener(object : IPackagesChangeListener {
                override fun onPackageAddOrReplace(packageName: String, versionCode: Int) {
                    ...
                    val appTasks = AppTaskDaoManager.instance.with(_taskManagerProvider.channel).gets_ofApkPackageName_satisfyApkVersionCode(packageName, versionCode)
                    Log.d(TAG, "onPackageAddOrReplace: appTasks $appTasks")
                    appTasks.forEach { appTask ->
                        /*_taskProviderSetInstall*/_taskManagerProvider.getTaskSetInstall()?.onTaskFinished(ATaskState.STATE_INSTALL_SUCCESS, appTask, ATaskName.TASK_INSTALL, STaskFinishType.SUCCESS)
                    }
                }

                override fun onPackageRemove(packageName: String) {
                    val appTasks = AppTaskDaoManager.instance.with(_taskManagerProvider.channel).gets_ofApkPackageName(packageName)
                    appTasks.forEach { appTask ->
                        /*_taskProviderSetUninstall*/_taskManagerProvider.getTaskSetUninstall()?.onTaskFinished(ATaskState.STATE_UNINSTALL_SUCCESS, appTask, ATaskName.TASK_UNINSTALL, STaskFinishType.SUCCESS)
                    }
                }
            })
        }
    ...
    private fun resetSelfState() {
        ...
        val appTask = _taskManagerProvider.getAppTaskDaoManager().get_ofApkPackageName_ApkVersionCode(packageName, UtilKPackage.getVersionCode(packageName, 0))
        if (appTask != null && appTask.isTaskProcess(_taskManagerProvider, ATaskName.TASK_INSTALL)) {
            _taskManagerProvider.onTaskFinish(appTask, ATaskName.TASK_INSTALL, STaskFinishType.SUCCESS)
        }
    }
}