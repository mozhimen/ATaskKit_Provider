package com.mozhimen.taskk.provider.task.download.okdownload
...
import com.mozhimen.taskk.provider.task.download.mos.DownloadProgressBundle
 ...
 * @Version 1.0
 */
abstract class TaskDownloadOkDownload(taskManager: ATaskManagerProvider, iTaskLifecycle: ITaskLifecycle) : ATaskDownload(taskManager,iTaskLifecycle) {
    ...
    override fun init(context: Context) {
        if (!hasInit()) {
            ...
            try {
                ...
                AppTaskDaoManager.instance.with(_taskManager.channel).gets_ofIsTaskDownloading().forEach {
...
//                return
    ...
    inner class InnerDownloadListener1(@ATaskNodeQueueName private val taskNodeQueueName: String) : DownloadListener1() {
        ...
        override fun taskStart(downloadTask: DownloadTask, model: Listener1Assist.Listener1Model) {
            ...
            val appTask = AppTaskDaoManager.instance.with(_taskManager.channel).get_ofTaskDownloadUrlCurrent(downloadTask.url) ?: return
