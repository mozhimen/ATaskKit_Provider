 ...
 * @Version 1.0
 */
class TaskOpenApk(taskManager: ATaskManagerProvider, iTaskLifecycle: ITaskLifecycle) : ATaskOpen(taskManager,iTaskLifecycle) {
    ...
    override fun taskStart(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        try {
            val boolean = UtilKContextStart.startContext_ofPackageName( appTask.apkPackageName,UtilKApplicationWrapper.instance.get())
            if (boolean) {
