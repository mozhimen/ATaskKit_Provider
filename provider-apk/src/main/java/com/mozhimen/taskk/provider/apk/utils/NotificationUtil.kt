...
object NotificationUtil {
    ...
    fun showNotification(
        ...
        if (UtilKBuildVersion.isAfterV_26_8_O()) {
            val notificationChannel = NotificationChannel(NETK_APP_NOTIFICATION_CHANNEL_ID, channelName, NotificationManager.IMPORTANCE_LOW)
        ...
        when {
            appTask.isTaskSuccess(taskManager, taskNodeQueueName) -> {
                intent?.let {
                    builder.setContentIntent(UtilKPendingIntentGet.getActivity_IMMUTABLE(0, it,context))
                }
            }
