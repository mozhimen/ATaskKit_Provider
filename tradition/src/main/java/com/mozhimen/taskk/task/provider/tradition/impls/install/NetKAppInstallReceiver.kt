                ...
                intent.dataString?.let { dataString ->
                    ...
                    when (intent.action) {
                        ...
                        CIntent.ACTION_PACKAGE_ADDED, CIntent.ACTION_PACKAGE_REPLACED -> {//有应用发生变化，强制刷新应用
                            ...
                            if (packageInfo != null) {
                                com.mozhimen.taskk.task.provider.tradition.impls.install.NetKAppInstallManager.onInstallSuccess(apkPackName, UtilKPackageInfo.getVersionCode(packageInfo))
                            } else {
                                com.mozhimen.taskk.task.provider.tradition.impls.install.NetKAppInstallManager.onInstallSuccess(apkPackName, -1)
