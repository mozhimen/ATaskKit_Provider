package com.mozhimen.taskk.task.provider.tradition.impls.verify
...
internal object NetKAppVerifyManager : IUtilK {
    ...
    fun verify(appTask: AppTask) {
        ...
        if (appTask.fileNameExt.endsWith(".apk") || appTask.fileNameExt.endsWith(".npk")) {//如果文件以.npk结尾则先解压
             ...
             */
            NetKApp.instance.onVerifyFail(appTask, CErrorCode.CODE_TASK_VERIFY_FORMAT_INVALID.intErrorCode2taskException())
    ...
    private fun verifyApp(appTask: AppTask) {
        ...
        if (com.mozhimen.taskk.task.provider.tradition.impls.unzip.NetKAppUnzipManager.isUnziping(appTask)) {
        ...
        val externalFilesDir = NetKApp.instance.getDownloadPath() ?: run {
             ...
             * [CNetKAppState.STATE_VERIFY_FAIL]
             */
            NetKApp.instance.onVerifyFail(appTask, CErrorCode.CODE_TASK_VERIFY_DIR_NULL.intErrorCode2taskException())
        ...
        if (!fileApk.exists()) {
             ...
             * [CNetKAppState.STATE_VERIFY_FAIL]
             */
            NetKApp.instance.onVerifyFail(appTask, CErrorCode.CODE_TASK_VERIFY_FILE_NOT_EXIST.intErrorCode2taskException())
        ...
        if (isNeedVerify(appTask)) {
            ...
            if (!TextUtils.equals(appTask.taskVerifyFileMd5, apkFileMd5Locale)) {
                 ...
                 * [CNetKAppState.STATE_VERIFY_FAIL]
                 */
                NetKApp.instance.onVerifyFail(appTask, CErrorCode.CODE_TASK_VERIFY_MD5_FAIL.intErrorCode2taskException())
                UtilKLogWrapper.e(TAG, "verifyAndUnzipNpk: download file fail")
    ...
    private fun onVerifySuccess(appTask: AppTask, fileApk: File) {
        ...
        com.mozhimen.taskk.task.provider.tradition.impls.unzip.NetKAppUnzipManager.unzip(appTask.apply {
            filePathNameExt = fileApk.file2strFilePath()
        })
    }
