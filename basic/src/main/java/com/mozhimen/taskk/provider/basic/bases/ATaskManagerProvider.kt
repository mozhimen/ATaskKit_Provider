...
import com.mozhimen.taskk.provider.basic.cons.STaskType
import com.mozhimen.taskk.provider.basic.db.AppTaskDaoProvider
 ...
 * @Version 1.0
 */
abstract class ATaskManagerProvider(val channel: String) : BaseUtilK(), ITask, ITaskEvent {
    ...
    open fun init(context: Context) {
        ...
        AppTaskDaoManager.instance.with(channel).init()
        getTaskSets().forEach {
            it.init(context)
        }
    ...
    fun getAppTaskDaoManager(): AppTaskDaoProvider =
        AppTaskDaoManager.instance.with(channel)
...
//    }
    ...
    fun getCurrTaskNode_ofTaskSet(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String, firstTaskNode_ofTaskNodeQueue: STaskNode? = null): STaskNode? {
    ...
    fun getCurrTaskNode_ofTaskNodeQueue(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String, firstTaskNode_ofTaskNodeQueue: STaskNode? = null): STaskNode? {
        val realTaskNode = getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName, firstTaskNode_ofTaskNodeQueue)
        val taskNodeQueue = getTaskNodeQueue(appTask.fileExt, taskNodeQueueName) ?: return null
        if (taskNodeQueue.contains(realTaskNode)) {
            return realTaskNode
        } else {
            taskNodeQueue.forEachIndexed { index, sTaskNode ->
                if (sTaskNode == realTaskNode) {
                    return realTaskNode
                } else if (sTaskNode is STaskNode.TaskNodeMulti && sTaskNode.taskNodes.contains(realTaskNode)) {
                    return sTaskNode
                }
            }
            return realTaskNode
    ...
    fun getNextTaskNode_ofTaskNodeQueue(@AFileExt fileExt: String, @ATaskNodeQueueName taskNodeQueueName: String, currTaskNode: STaskNode): STaskNode? {
        ...
        UtilKLogWrapper.d(TAG, "getNextTaskNode_ofTaskNodeQueue: taskNodeQueue $taskNodeQueue currTaskNode $currTaskNode")
        var currentIndex = -1//taskNodeQueue.indexOf(currTaskNode)
        run loop@{
            taskNodeQueue.forEachIndexed { index, sTaskNode ->
                if (sTaskNode == currTaskNode) {
                    currentIndex = index
                    return@loop
                } else if (sTaskNode is STaskNode.TaskNodeMulti && sTaskNode.taskNodes.contains(currTaskNode)) {
                    currentIndex = index
                    return@loop
                }
            }
        ...
        if (currentIndex < 0) {
            UtilKLogWrapper.d(TAG, "getNextTaskNode_ofTaskNodeQueue: currentIndex $currentIndex")
            return null
        }
    ...
    fun taskStart(appTaskInner: AppTask, @ATaskNodeQueueName taskNodeQueueName: String, taskType: STaskType, @ATaskState taskState: Int, listener: IA_Listener<AppTask>) {
        registerTaskListener(object : ITasks {
            ...
            override fun onTaskDownloading(appTask: AppTask, progress: Int, currentIndex: Long, totalIndex: Long, offsetIndexPerSeconds: Long) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.DOING && taskState.getTaskCode() == ATaskState.STATE_DOWNLOAD_CREATE) {
                    unregisterTaskListener(this)
            ...
            override fun onTaskDownloadCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_DOWNLOAD_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskDownloadSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_DOWNLOAD_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskDownloadFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_DOWNLOAD_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskVerifyCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_VERIFY_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskVerifySuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_VERIFY_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskVerifyFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_VERIFY_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskUnzipCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_UNZIP_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskUnzipSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_UNZIP_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskUnzipFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_UNZIP_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskInstallCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_INSTALL_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskInstallSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_INSTALL_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskInstallFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_INSTALL_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskOpenCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_OPEN_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskOpenSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_OPEN_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskOpenFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_OPEN_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskCloseCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_CLOSE_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskCloseSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_CLOSE_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskCloseFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_CLOSE_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskUninstallCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_UNINSTALL_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskUninstallSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_UNINSTALL_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskUninstallFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_UNINSTALL_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskDeleteCancel(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.CANCEL && taskState.getTaskCode() == ATaskState.STATE_DELETE_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
            ...
            override fun onTaskDeleteSuccess(appTask: AppTask) {
                if (appTaskInner.id == appTask.id && taskType == STaskType.SUCCESS && taskState.getTaskCode() == ATaskState.STATE_DELETE_CREATE) {
                    unregisterTaskListener(this)
            ...
            override fun onTaskDeleteFail(appTask: AppTask, exception: TaskException) {
                if (appTaskInner.id == appTask.id && taskType is STaskType.FAIL && taskState.getTaskCode() == ATaskState.STATE_DELETE_CREATE) {
                    unregisterTaskListener(this)
                    listener.invoke(appTask)
                }
            }
    ...
    override fun taskStart(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        ...
        UtilKLogWrapper.d(TAG, "taskStart: currTaskNode $currTaskNode nextTaskNode $nextTaskNode")
...
//                getNextTaskSet(appTask.fileExt, taskNodeQueueName, currTaskName)?.taskStart(appTask, taskNodeQueueName)
    ...
    override fun taskResume(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        ...
        getTaskSet(getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName)?.taskName ?: return)?.taskResume(appTask, taskNodeQueueName)
    }
    ...
    override fun taskPause(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        ...
        getTaskSet(getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName)?.taskName ?: return)?.taskPause(appTask, taskNodeQueueName)
    }
    ...
    override fun taskCancel(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        ...
        getTaskSet(getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName)?.taskName ?: return)?.taskCancel(appTask, taskNodeQueueName)
    }
    ...
    override fun canTaskResume(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String): Boolean {
        ...
        return getTaskSet(getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName)?.taskName ?: return false)?.canTaskResume(appTask, taskNodeQueueName) ?: false
    }
    ...
    override fun canTaskPause(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String): Boolean {
        ...
        return getTaskSet(getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName)?.taskName ?: return false)?.canTaskPause(appTask, taskNodeQueueName) ?: false
    }
    ...
    override fun canTaskCancel(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String): Boolean {
        ...
        return getTaskSet(getCurrTaskNode_ofTaskSet(appTask, taskNodeQueueName)?.taskName ?: return false)?.canTaskCancel(appTask, taskNodeQueueName) ?: false
    }

    /////////////////////////////////////////////////////////////////

    override fun onTaskCreate(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String, isUpdate: Boolean) {
        appTask.toTaskStateNew(if (isUpdate) AState.STATE_TASK_UPDATE else AState.STATE_TASK_CREATE, appTask.taskChannel)
    ...
    override fun onTaskUnavailable(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        appTask.toTaskStateNew(AState.STATE_TASK_UNAVAILABLE, appTask.taskChannel)
        _iTaskLifecycle.onTaskStarted(appTask.taskState, appTask, taskNodeQueueName)
    }
...
//    }
    ...
    override fun onTaskFinish(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String, finishType: STaskFinishType) {
        appTask.toTaskStateNew(
            when (finishType) {
                STaskFinishType.SUCCESS -> getLastTaskNode_ofTaskNodeQueue(appTask.fileExt, taskNodeQueueName)?.taskName?.taskName2taskState()?.plus(AState.STATE_TASK_SUCCESS)
    ...
    fun onTask_ofOther(appTask: AppTask, @ATaskNodeQueueName taskNodeQueueName: String) {
        ...
        if (appTask.isAnyTaskSuccess()) {
            var currTaskNode = getCurrTaskNode_ofTaskNodeQueue(appTask, taskNodeQueueName) ?: return
            var nextTaskNode = getNextTaskNode_ofTaskNodeQueue(appTask.fileExt, taskNodeQueueName, currTaskNode)
            Log.d(TAG, "onTask_ofOther: currTaskNode $currTaskNode nextTaskNode $nextTaskNode")
            if (nextTaskNode != null && nextTaskNode != STaskNode.TaskNodeRestart) {
                if (currTaskNode is STaskNode.TaskNodeMulti && nextTaskNode is STaskNode.TaskNodeMulti) {
                    Log.d(TAG, "onTask_ofOther: currTaskNode is STaskNode.TaskNodeMulti && nextTaskNode is STaskNode.TaskNodeMulti")
                    if (currTaskNode.taskNodes.contains(STaskNode.TaskNodeBlocker)) {
                        UtilKLogWrapper.d(TAG, "onTask_ofOther: currTaskNode.taskNodes.contains(STaskNode.TaskNodeBlocker)")
                        return
                    }
                    if (nextTaskNode.taskNodes.isNotEmpty() && nextTaskNode.taskNodes.firstOrNull() != null) {
                        getTaskSet(nextTaskNode.taskNodes.first().taskName)?.taskStart(appTask, taskNodeQueueName)
                    }
                } else if (currTaskNode is STaskNode.TaskNodeMulti && nextTaskNode !is STaskNode.TaskNodeMulti) {
                    Log.d(TAG, "onTask_ofOther: currTaskNode is STaskNode.TaskNodeMulti && nextTaskNode !is STaskNode.TaskNodeMulti")
