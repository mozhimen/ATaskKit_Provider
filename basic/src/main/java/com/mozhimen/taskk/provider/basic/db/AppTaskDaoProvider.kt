 ...
 * @Version 1.0
 */
class AppTaskDaoProvider(private val _channel: String) : IUtilK {
    ...
    fun init() {
        ...
        TaskKExecutor.execute(TAG + "init") {
            _appTasks.putAll(AppTaskDb.getAppTaskDao().gets_ofAll(_channel).associateBy { it.id })
    ...
    fun gets_ofIsTaskProcess(taskManager: ATaskManagerProvider, @ATaskNodeQueueName taskNodeQueueName: String): List<AppTask> {
    ...
    fun gets_ofIsTaskSuccess(taskManager: ATaskManagerProvider, @ATaskNodeQueueName taskNodeQueueName: String): List<AppTask> {
